...
_HOST_GUEST_RE = re.compile(r'^(host|guest)_[0-9]+$')
...
class Feeder(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        if debug_samples:
            rng = np.random.RandomState(0)  # 固定种子，方便复现
            n = min(int(debug_samples), len(sample_ids))
            idxs = rng.choice(len(sample_ids), size=n, replace=False)
    ...
    def _to_traj_array(v, sid: str, path: str) -> np.ndarray:
        """
        把任意 ball value 转成 (T,3) float32。
        兼容 list/tuple、ndarray、以及 dict 内嵌 traj 字段。
        """
        if v is None:
            raise ValueError("ball value is None")

        # 1) dict 内嵌
        if isinstance(v, dict):
            for k in ["traj", "ball_traj", "ball", "xyz", "pos", "positions", "data"]:
        ...
        if arr.ndim != 2 or arr.shape[1] < 3:
            raise ValueError(f"unexpected ball traj shape: {arr.shape} @ {sid} ({path})")

        arr = arr.astype(np.float32)
        return arr[:, :3]


    def _merge_trajs(trajs: list[np.ndarray]) -> np.ndarray:
        """
        多条候选轨迹合成一条：
        - 对齐到最短长度
        - nan-safe mean
        ...
        stack = np.stack([t[:L] for t in trajs], axis=0)  # (K,L,3)
    ...
    def _load_ball_traj(self, sid: str) -> np.ndarray:
        ...
        if isinstance(d, dict):
            for k in ["traj", "ball_traj", "ball", "xyz", "pos", "positions", "data"]:
                if k in d and d[k] is not None:
                    return _to_traj_array(d[k], sid, path)

            # 情况 B：顶层 dict 的 key 是 host_x / guest_x
            hg_keys = [k for k in d.keys() if isinstance(k, str) and _HOST_GUEST_RE.match(k)]
            if hg_keys:
                trajs = []
                for k in hg_keys:
                    try:
                        trajs.append(_to_traj_array(d[k], sid, path))
                    except Exception:
                        continue
                if not trajs:
                    raise ValueError(f"ball dict has host/guest keys but none parseable: {sid} ({path})")
                return _merge_trajs(trajs)

            # 兜底：顶层 dict 但完全不认识
            raise ValueError(f"unrecognized ball dict keys: {list(d.keys())[:10]} @ {sid} ({path})")

        # 情况 C：不是 dict，直接当数组
        return _to_traj_array(d, sid, path)


    def __getitem__(self, idx):
        sid = self.sample_ids[idx]

        # --- load joints, and remember player key order ---
        jp = self._joints_path(sid)
        obj = np.load(jp, allow_pickle=True)

        self._last_player_keys = None
        if isinstance(obj, np.ndarray) and obj.ndim == 0 and obj.dtype == object:
            d = obj.item()
            if isinstance(d, dict):
                self._last_player_keys = list(d.keys())

        x = _load_array_fix_shape(obj)    # (C,T,V,M)  C should be 3
        ...
        if C == 3:
            ...
            if C > 4:
                ...
                padc = 4 - C
                x = np.concatenate([x.astype(np.float32), np.zeros((padc, T, V, M), dtype=np.float32)], axis=0)

        # clear C4
        x[3].fill(0.0)

        # --- fill C4 from ball keys (clip-level one-hot) ---
        bd = None
        try:
            ball_path = self._ball_path(sid)
            with open(ball_path, "rb") as f:
                bd = pkl.load(f)

            if isinstance(bd, dict) and self._last_player_keys is not None:
                keyset = set(bd.keys())
                for m, pk in enumerate(self._last_player_keys[:M]):
                    if pk in keyset:
                        root_v = 11
                        x[3, :, root_v, m] = 1.0
            else:
                # ball pkl 不是 dict 或 joints 不是 dict：保持全0
                pass

        except Exception as e:
            # 只打印一次，避免刷屏
            if not hasattr(self, "_ball_warned"):
                self._ball_warned = False
            if not self._ball_warned:
                print(f"[FeederSGA WARN] ball parse failed once: sid={sid} err={repr(e)}")
                self._ball_warned = True

        label = self.labels[idx]

        if getattr(self, "debug", False):
            if bd is not None and isinstance(bd, dict):
                print("[FeederSGA DEBUG] sid", sid, "ball keys:", list(bd.keys()))
            print("[FeederSGA DEBUG] sid", sid, "player_keys:", self._last_player_keys)
            print("[FeederSGA DEBUG] sid", sid, "x.shape =", x.shape)
            print("[FeederSGA DEBUG] sid", sid, "C4 per-player sum:", x[3].sum(axis=(0, 1)).tolist())
            print("[FeederSGA DEBUG] sid", sid, "C4 stats:", float(x[3].min()), float(x[3].max()), float(x[3].mean()), float(x[3].std()))


        # x 现在可能是 (4,T,V,M) 或 (3,T,V,M)
        if not getattr(self, "use_c4", True):
            x = x[:3]  # ✅ 强制 3 通道

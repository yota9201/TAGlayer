...
class Processor(IO):
    ...
    def train(self):
        ...
        self.epoch_info['mean loss'] = mean_loss
        self.io.print_log(f"\tEpoch {self.meta_info['epoch']} finished | mean loss: {mean_loss:.4f}")
    ...
    def test(self):
        ...
        mean_loss = total_loss / max(1, num_batches)
        self.epoch_info['mean loss'] = mean_loss
        self.io.print_log(f"\tEval epoch {self.meta_info['epoch']} finished | mean loss: {mean_loss:.4f}")
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if ((epoch + 1) % self.arg.eval_interval == 0) or (
                    ...
                    self.test()
                    self.io.print_log('Done.')
            # === 在训练阶段全部结束后，打印一次 best 总结 ===
            if self.arg.phase == 'train':
                try:
                    if hasattr(self, 'best_top1'):
                        import os
                        best_epoch = int(getattr(self, 'best_epoch', -1))
                        best_top1  = float(getattr(self, 'best_top1', -1.0)) * 100.0
                        best_loss  = float(getattr(self, 'best_loss', float('inf')))
                        best_path  = os.path.join(self.work_dir, 'best_model.pt')
                        self.io.print_log(
                            f"[BEST][Summary] epoch {best_epoch} | Top1={best_top1:.2f}% | loss={best_loss:.4f}"
                        )
                        if os.path.exists(best_path):
                            self.io.print_log(f"[BEST] checkpoint: {best_path}")
                except Exception as e:
                    self.io.print_log(f"[BEST] summary failed: {e!r}")
